...
class bank:
    ...
    def nac(self):
        name = input("請入您的姓名: ")
        firs_bank.update = name
        money = int(input("請存入現金開戶: "))
        firs_bank.update = money
        pword = input("請設定密碼: ")
        firs_bank.update = pword
    ...
    def say_hi(self):
        print ("Hello!!{0}先生，開戶日期為{3}帳號{1}，\
        目前已存款{2}元".format(self.name,self.id,self.money,time.strftime("%m/%d/%Y")))
        
    def ck(self):
        global x
        x = int(input("{}請按0、{}請按1、{}請按2、{}請按3\
        、{}請按4、{}請按9 ".format("提款","存款","查詢餘額","設定密碼","重新開戶",'離開')))
        if (x == 0):
            self.pu()
        elif (x == 1):
            self.sa()
